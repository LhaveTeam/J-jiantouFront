	...
	(function() {
		...
		window.jt = window.LH = {
			...
			init: function(BarType, packageName) {
				...
				var topBar = LH.tagName('header')['0'];
				gM = mui;
			...
			newInterface: function(jsonData) {
				if(jsonData.styles == undefined) {
					...
					if(jsonData.styles.bottom == undefined) {
						...
						jsonData.styles.height = LH.getScreenInfo('height') - parseInt(jsonData.styles.bottom) + 'px';
					}
			...
			sendNotice: function(targetId, fnName, jsonData) {
				if(targetId.constructor == Array) {
					for(var i = 0; i < targetId.length; i++) {
						if(targetId[i] == 'root') {
							LH.getStartInterface(function(targetObject) {
								gM.fire(targetObject, fnName, jsonData);
							});
						} else {
							LH.getTargetInterface(targetId[i], function(targetObject) {
								gM.fire(targetObject, fnName, jsonData);
							});
						}
					}
					...
					if(targetId == 'root') {
						...
						LH.getTargetInterface(targetId, function(targetObject) {
							gM.fire(targetObject, fnName, jsonData);
						});
					}
				}
								...
								LH.camera({
									succFn: function(path, name) {
										if(jsonData && jsonData.isUpload) {
											LH.uploadFiles(jsonData.uploadUrl, path, function(imgPath) {
			...
			createChildInterface: function(jsonData) {
				...
				if(jsonData.styles == undefined) {
					...
					if(jsonData.styles.bottom == undefined) {
						...
						jsonData.styles.height = LH.getScreenInfo('height') - parseInt(jsonData.styles.bottom) - parseInt(jsonData.styles.top) + 'px';
					}
					if(jsonData.styles.width == undefined) {
						jsonData.styles.width = "100%";
					}
			...
			uploadFiles: function(uploadUrl, filePath, succFn, errFn) {
				...
				LH.showWaiting('上传中...');
				var task = gP.uploader.createUpload(uploadUrl, { method: "POST" },
					function(t, status) {
			...
			openUrl: function(targetUrl) {
				LH.getRuntime().openURL(targetUrl);
			},
			/**
			...
			setStartpage: function(jsonData) {
				...
				var vH = LH.getScreenInfo('height');
			...
			sideslipNav: function(type, showObjId, closeObjId) {
				...
				var offCanvasSide = LH.id("offCanvasSide");
			 ...
			 */
			indexes: function() {
				var header = LH.tagName('header')['0'];
				var list = LH.id('list');
			...
			slideNav: function(fn) {
				var LH_nav = document.getElementById('LH_nav');
				var LH_nav_ul = document.getElementById('LH_nav_ul');
				var LH_nav_li = LH_nav.getElementsByTagName('li');
				var oWidth = LH_nav_li['0'].offsetWidth;
				var oLeft = LH_nav_li['0'].offsetLeft;
				var down_line = document.getElementById('down_line');
				var ulWidth = (oWidth * LH_nav_li.length) + (oLeft * 2);
				LH_nav_ul.style.width = ulWidth + 'px';
				down_line.style.width = oWidth + 'px';
				var slide_data = 0;
				var screenWidth = 0;
				var target = 0;
				LH_nav_ul.ontouchstart = function(ev) {
					var vW = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
					var touch = ev.touches['0'];
					var oldX = touch.clientX;
					var oldLeft = LH_nav_ul.offsetLeft;
					LH_nav_ul.ontouchmove = function(ev) {
						var touch = ev.touches['0'];
						var newX = touch.clientX;
						target = newX - oldX;
						var currentLeft = parseInt(LH_nav_ul.offsetLeft);
						if(target > 0 && currentLeft >= 0) {
							LH_nav_ul.style.left = 0 + 'px';
							return;
						} else {
							if(-currentLeft >= ulWidth - vW && target < 0) {
								LH_nav_ul.style.left = vW - ulWidth + 'px';
							} else {
								LH_nav_ul.style.left = oldLeft + target + 'px';
							}
						}
						ev.preventDefault();
					}
				}
				for(var a = 0; a < LH_nav_li.length; a++) {
					(function(index) {
						LH_nav_li[a].onclick = function() {
							for(var s = 0; s < LH_nav_li.length; s++) {
								LH_nav_li[s].className = '';
							}
			...
			share: function(jsonData) {
				...
				function updateSerivces() {
					plus.share.getServices(function(s) {
						...
						LH.closeWaiting();
						LH.prompt("获取分享服务列表失败：" + e.message);
					});
				}
				gM.ready(function() {
					...
					function shareAction(id, ex) {
						...
						if(s.authenticated) {
							...
							s.authorize(function() {
								...
								LH.prompt("认证授权失败：" + e.code + " - " + e.message);
							});
						}
					...
					function listFilter(arr) {
						...
						for(var i = 0; i < arr.length; i++) {
							...
							if(/^[\u4e00-\u9fa5]+$/.test(name)) { //是中文
								var first = LH.getFirstLetter(name);
							} else {
								//判断是否为特殊字符
								if(name) {
